...
begin
	...
	size_max = 20
	for i ∈ eachindex(test_set[!,5])
		smiles_string = test_set[i,5]
		allowed_atoms = ['C','O','N','c','S','B','P','F','o','I','K']
		disallowed_features = ['.','+']
		if count(n ∈ allowed_atoms for n ∈ smiles_string) >= size_max && all([i ∉ disallowed_features for i ∈ j])
			try 
			catch e
	
	cleaned_data = filter(j -> (count(i -> (
		i ∈ ['C','O','N','c','S','B','P','F','o','I','K']), j).<21 &&
		all([i ∉ ['H','.','+'] for i ∈ j]))
		, test_set[!,5])
end

# ╔═╡ f769aff5-d7d9-486e-a199-476561b4bc65
for i ∈ cleaned_data
	@show i
	smilestomol(i)
end
